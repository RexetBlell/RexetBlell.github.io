...
$("#btn_instructions").click(function() {
    ...
    message += "<p>Here’s how to use this practically. Add a key of your friend with a delay of 1 year. Add a key of some family member with a delay of 6 months. Add your hot wallet key (such as Metamask or Parity) with a delay of 2 months. Add your hardware wallet key (such as Ledger Nano S) with a delay of 2 weeks. Add your most secure key, such as a paper wallet and set the delay to 0. The idea is that the lower the security of the key, the higher the delay should be.</p>"
    message += "<p>If one of the keys gets hacked (or becomes malicious, in case of family or friend), you can always cancel their transactions (and remove that user), using a key with with a lower delay (which means it has more privileges). You would need to keep an eye on the wallet from time to time to make sure there are no unexpected pending transactions in it.</p>"
